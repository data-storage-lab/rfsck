...
echo "cp ${IMG_BS} ${TMP_FILE}"
cp ${IMG_BS} ${TMP_FILE}
sleep 2

echo -e "\n### replay for splitting IO"

echo "$REPLAYER --disk-file ${TMP_FILE} --io-header-log ${FSWKLD_IO_HEAD_LOG} --io-header-split-log ${FSWKLD_IO_HEAD_SPLIT_LOG}  --io-data-log ${FSWKLD_IO_DATA_LOG} --fail-type 0 --start-io  1 --end-io ${REPLAY_END_IO} --pfe-err-blk ${ERR_BLOCK_LOG}  --split-io 1"
