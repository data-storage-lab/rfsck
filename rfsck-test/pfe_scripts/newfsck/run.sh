...
#while [ $it -le $it_num ]; do
    ...
    echo "$1 : $PFE_LOG_DIR" >> $abshere/add_text.txt
...
#COMMENT1
    export COPY_FILE=$IMG_DIR/img.128M.empty.ext4.fswkld.copy
    echo "COPY_FILE: $COPY_FILE"
    # Step 1: Run and record FSCK1_Y
    $abshere/1-rec-fsck1-y.sh 2>&1 | tee ${PFE_LOG_DIR}/log.1-run-fsck1-y
    
    # Step 2: Replay and cut FSCK1_Y
    $abshere/2-rep-cut-fsck1.sh 2>&1 | tee ${PFE_LOG_DIR}/log.2-rep-cut-fsck1
    # Step 3: Run FSCK2_Y
    ...
    #$abshere/app-fsck2.sh 2>&1 | tee ${PFE_LOG_DIR}/log.3-run-fsck2-y
        
    #FINAL_FILE=~/pfe_images/img.128M.empty.ext4.final
    #echo "Executing cp $FSCK2_DEV $FINAL_FILE"
    #cp $FSCK2_DEV $FINAL_FILE
