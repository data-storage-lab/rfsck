...
static errcode_t PRS(int argc, char *argv[], e2fsck_t *ret_ctx)
{
	...
	*ret_ctx = ctx;
	...
	while ((c = getopt(argc, argv, "panyrcC:B:dE:fvtFVM:b:I:j:P:l:L:N:SsDkz:R:")) != EOF)
		switch (c) {
                case 'R':
                        ctx->redo_file = optarg;
                        redo_file = optarg;
                        break;
		case 'C':
			ctx->progress = e2fsck_update_progress;
...
/*        if(ctx->redo_file) {
            is_redo_enabled = 1;
            struct unix_private_data unix_data = (struct unix_private_data *)fs->io->private_data;
            retval = redo_open(ctx->redo_file, unix_data->dev, flags);
            if(retval) {
