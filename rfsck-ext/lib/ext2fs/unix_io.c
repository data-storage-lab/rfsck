	...
	if ((channel->align == 0) ||
	    (IS_ALIGNED(buf, channel->align) &&
	     IS_ALIGNED(size, channel->align))) {
                ...
                if(is_redo_enabled) {
                    actual = rfsck_write(location, size, buf);
                    return actual;
                } else {
...
#define RLIM_INFINITY  (~0UL)
	...
	*channel = io;
        ...
        if(is_redo_enabled) {
            int dev_fdesc = open(name, O_RDWR, 0666);
            retval = rfsck_open(redo_file, dev_fdesc, flags);
            if(retval) {
                printf("Unable to open REDO file: %s\n", redo_file);
                printf("Disbale logging\n");
                is_redo_enabled = 0;
            }
        }
			   ...
			   io_channel *channel)
{
	...
	if (flags & IO_FLAG_DIRECT_IO)
		open_flags |= O_DIRECT;
#endif
	fd = ext2fs_open_file(name, open_flags, 0666);
	if (fd < 0)
		return errno;
				 ...
				 int size, const void *buf)
{
	...
	if ((retval = flush_cached_blocks(channel, data, 1)))
        ...
        if(is_redo_enabled) {
            actual = rfsck_write(offset + data->offset, size, buf);
            return actual;
        } else {
