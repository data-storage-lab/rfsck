...
retry:
	...
	if (fstat(fd, &statb) < 0) {
        ...
        // added for redo log
        //redo_open("/home/dsl/project/redo_api/test/test.1.redo", fd, flags);

//        int retval =  redo_open("/dev/sdd", fd, flags);
//         int retval =  redo_open("/home/dsl/pfe_images/img.128M.empty.ext4.undo", fd, flags);
          if(redo_file) {
              int devd_fd = open(path, O_RDWR, 0666);
              int retval = rfsck_open(redo_file, devd_fd, flags);
